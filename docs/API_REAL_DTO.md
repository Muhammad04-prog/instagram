...
## Swagger v4 (19.07.2026) — 190 → 230

Ҳамон ҳост, endpoint нест нашуд. 40 нав: 2FA + сессияҳо, insights (пост/сторис/аккаунт),
черновик/нашри мунтазир, pin (пост/шарҳ), приватнокии пост, соавторҳо, ремикс, ревюи қайд, Add Yours,
стикерҳои интерактивӣ дар сторис, vanish/view-once дар чат, `settings` (тег нав), `GET /feed`.

🔴 **`POST /auth/login` — ҷавоб барои 2FA дар Swagger тасвир НАШУДААСТ.** `TwoFactorRequiredDto`
(`{ twoFactorRequired: true, ticket: string }`) аз рӯи тавсифи `Verify2faDto.ticket`
("Тикет, выданный при логине (twoFactorRequired)") **гумонзада** шуд — 200-и ҳуҷҷатии
`/auth/login` то ҳол танҳо `TokensDto`-ро эълон мекунад, бидуни шохаи 2FA. Санҷиши зинда имконнопазир
буд (БД дар вақти ин кор дастрас набуд/санҷида нашуд бо ин flow). Код ба номи майдони `twoFactorRequired`
такя намекунад — фарқкунӣ бо `"ticket" in response` аст, зеро маҳз ҳамин майдонро `Verify2faDto`
воқеан истифода мебарад. **Пеш аз такя кардан ба ин шакл — бо аккаунти 2FA-фаъол зинда санҷед.**

`SessionDto`-ро надорад: майдони "охирин фаъолият" (last active) — танҳо `createdAt`/`expiresAt` ҳаст.
`AuthUserDto` майдони "2FA фаъол аст?" надорад — `TwoFactorSettings` ҳолати воқеиро дар боркунии
саҳифа гирифта наметавонад, танҳо амалҳои дохили як ташриф пайгирӣ мекунад.
