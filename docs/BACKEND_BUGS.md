...
⚠️ 2026-07-14: ҳамин суетӣ ҳоло баъзан ба **400 Bad Request** мерасад — на 200 бо
лентаи пурра. Тафтиш карда шуд, ки `src/app/api/proxy/[...path]/route.ts` танҳо
proxy аст (ягон validation/timeout аз тарафи мо нест), пас 400 бевосита аз худи
бэкенд меояд, баъд аз ~26 сонияи интизорӣ (server console: `application-code: 26.3s`).
Эҳтимол, ҳарчи лентаи обунашудагон калонтар мешавад, дархост ба лимити вақти
худи сервер бармехӯрад.

✅ `get-posts` ва `get-reels` пагинатсияро дуруст иҷро мекунанд — танҳо ҳамин endpoint шикастааст.

**Ҳалли ниҳоӣ (2026-07-14):** азбаски `get-following-post` дуруст кор намекунад, `useFeed()`
(`src/hooks/usePosts.ts`) дигар онро **тамоман фарқ намезанад**. Ба ҷои он:

1. `FollowingRelationShip/get-subscriptions` — рӯйхати корбароне, ки ман обуна ҳастам.
2. Барои ҳар корбар, параллел, `Post/get-posts?UserId=…` (ин endpoint дуруст ва тез кор мекунад).
3. Натиҷаҳо якҷоя ва бо `datePublished` (нав → кӯҳна) сорт карда мешаванд.

Агар боркунии як муаллиф ноком шавад (масалан профили ҳазфшуда), он `[]` бармегардонад ва
боқимонда лента вайрон намешавад. Ин ҳам суетии 26-сонияро, ҳам 400-ро тамоман канор мезанад.
...
## 23. `get-reels` баъзан постҳои саҳифаи қаблиро такрор медиҳад 🟡

Дар браузер (Console) React огоҳии «two children with the same key» доданд — `postId` 84 ва 79 дар
ду саҳифаи гуногуни `get-reels` пайдо шуданд. Баръакси #21 (ки `get-following-post` пагинатсияро
**тамоман нодида мегирад**), ин ҷо танҳо баъзе элементҳо такрор мешаванд — эҳтимолан аз сабаби
sort-и ноустувор (instable ordering) байни дархостҳо, на аз сабаби skip/take вайрон.

**Ҳалли мо:** `useReels()` (`src/hooks/usePosts.ts`) ду ҳимоя дорад:

1. `select` пеш аз render бо `postId` dedupe мекунад — калиди такрории React ҳаргиз намебарояд.
2. `getNextPageParam` дархости саҳифаи навбатиро **қатъ мекунад**, агар саҳифаи охирин ягон
   `postId`-и нав наоварда бошад (то ҳалқаи беохири fetch пешгирӣ шавад).

✅ Агар бэкенд sort-ро устувор кунад — рафтори UI бе тағйирот дуруст мемонад.

---

## 24. `Story/AddStories` метавонад пайвастшавиро партояд (`ERR_CONNECTION_RESET`) 🟡

Ҳангоми боркунии файли калон ба `POST /Story/AddStories` баъзан браузер на 502/400,
балки худи `net::ERR_CONNECTION_RESET`-ро нишон медиҳад. Бо `curl` такрор карда шуд:
сервери воқеӣ пайвастро **пеш аз анҷоми боркунии тамоми файл** мебандад (масалан ҳангоми
хатои auth ё маҳдудияти андоза). Прокси-и мо (`src/app/api/proxy/[...path]/route.ts`)
дар ин ҳолат ба таври дуруст `502` бармегардонад, вале браузер (fetch/XHR, бар хилофи
`curl`, ки `Expect: 100-continue` мефиристад) аксар вақт ин канда шудани пайвастро
ҳамчун хатои шабакавӣ (на ҷавоби 502) месанҷад — аз ин ҷо `ERR_CONNECTION_RESET` дар
Console.

**Аҳамият:** ин **хатои JS нест** — `useAddStory`-и мо (`src/hooks/useStories.ts`)
онро дар `onError` дуруст мегирад ва toast нишон медиҳад, боркунии файлҳои дигар идома
меёбад (`StoryUploadDialog.onSubmit`). UI намеафтад.

**Ислоҳи ёфташуда дар рафти санҷиш:** дар `src/lib/axios.ts` барои хатоҳои шабакавии
беҷавоб (status 0, монанди ҳамин) паём `error.message`-и худи axios буд — сатри
инглисии hardcode-и «Network Error», ки ба забонҳои дигар тарҷума намешуд (зидди
қоидаи «матни hardcode манъ»). Ҳоло дар чунин ҳолат паём холӣ мемонад, то ҳама 22
ҷои `toast.error(error.message || t("network"))` ба матни локализатсияшуда гузаранд.

---

# ҶАМЪБАСТ — 24 боги ёфташуда
...
| 23  | `get-reels` баъзан постҳои такрориро медиҳад        | 🟡     | Dedupe + қатъи пагинатсияи такрорӣ дар useReels   |
| 24  | `AddStories` метавонад `ERR_CONNECTION_RESET` диҳад | 🟡     | `useAddStory` toast мекунад, боркунӣ идома меёбад |
