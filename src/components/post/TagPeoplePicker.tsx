...
/**
 * "Tag people" — `taggedUserIds` on `POST /posts`.
 *
 * New: softclub had no tagging, which is why the profile's "Tagged" tab was a
 * placeholder for ten phases. Tagged posts land on `/profile/{id}/tagged`.
 */
  ...
  return (
    <div className="border-ig-separator relative border-t py-3">
      <div className="flex items-center gap-2">
        <UserPlus className="text-ig-text-secondary size-4 shrink-0" />
        <input
          value={term}
          onChange={(event) => setTerm(event.target.value)}
          placeholder={t("tagPeople")}
          aria-label={t("tagPeople")}
        ...
        <ul className="mt-2 flex flex-wrap gap-1.5">
          {value.map((user) => (
            ...
            >
              ...
              <button
                type="button"
                onClick={() => onChange(value.filter((tagged) => tagged.id !== user.id))}
