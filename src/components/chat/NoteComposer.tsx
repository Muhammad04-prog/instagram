...
/**
 * Write my note — takes over only the right pane of /chat, same as
 * ChatWindow/ChatEmptyState (ChatShell renders it in their place), not a
 * page-covering modal: real IG keeps the conversation list visible on the left.
 *
 * Editing and deleting a *live* note happen from the compact popover
 * (NoteOwnCard) instead — this screen only ever creates or replaces.
 */
export function NoteComposer() {
  const t = useTranslations("note");
  const tCommon = useTranslations("common");
  const { data: profile } = useMyProfile();
  const note = useNoteComposerStore((s) => s.editingNote);
  const close = useNoteComposerStore((s) => s.closeComposer);
  ...
  return (
    <div className="bg-ig-bg flex h-full flex-1 flex-col">
